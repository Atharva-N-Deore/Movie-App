    ...
    return (
        <Link href = {`/movies/${id}`} asChild>
            <TouchableOpacity className='w-[30%]'>
                ...
                <View className='flew-row items-center justify-start gap-x-1' style={{flexDirection:'row', alignItems:'center'}}>
                    <Image source={icons.star} className='size-4'/>
                    <Text className='text-xs text-white font-bold uppercase'>{Math.round(10*(vote_average/2))/10}/5</Text>
                </View>
                <View className='flex-row items-center justify-between'>
                    ...
                    {/* Useful to differentiate whether the card is showing a movie or TV series or whatever */}
                </View>
            </TouchableOpacity>
        </Link>
    )
